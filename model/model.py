...
class Model:
    ...
    """
    fillDD con lista e load nel model e ciclo e append per il dropdown nel controller
    Per gli archi con due nodi e anno in input che restituisce il peso controllare terza query
    Per ordinare gli archi in base al peso sorted(self._grafo.edges(data=True), key=lambda x: x[2]['weight'], reverse=True)
    Per samapre i nodi che compiaiono più di una volta guardare handleGraph.
    """